classdef Data < handle
    ...
    properties
        ...
        imageDirectory       % data location relative to the current directory
        matDirectory
    ...
    methods
        ...
        function D = Data(setupParameters)
            ...
            D.imageDirectory     = [setupParameters.directory '\Images'];
            D.matFilename   = [D.imageFilename(1:end-4) '.mat'] ;
            D.matDirectory = [setupParameters.directory '\Buildings'];
        ...
        function D = loadImageData(D)
            ...
            D.imageData = imread(fullfile(D.imageDirectory,D.imageFilename)) ;
        ...
        function D = loadLabels(D)
            ...
            polygonData = load(fullfile(D.matDirectory,D.labelFilename)) ;
        ...
        function viewImage(D)
            % Load the image
            image(D.imageData(:,:,1:3)) ; axis image ;
        end
        ...
        %------------------------------------------------------------------
        function saveData(D)
            matFullFile = fullfile(D.matDirectory,D.matFilename) ;
            save(matFullFile,'D')
        end
        ...
        function D = loadData(D)
            ...
            D.imageDirectory         = StoredData.D.imageDirectory ;
            D.matDirectory = StoredData.D.matDirectory;
        ...
        function D = checkIfFileExists(D)
            ...
            if ~(exist(D.imageDirectory, 'dir') == 7)
                mkdir(D.imageDirectory) ;
            end
